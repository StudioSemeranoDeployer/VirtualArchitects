...
const params = {
  ...
  modelZ: -5,
  modelScale: 1,
  selectedLocalModel: 'nessuno',
  localModels: ['nessuno', 'scultura.glb', 'architettura.glb'],
  urlSlot1: '',
  urlSlot2: '',
  urlSlot3: '',
  // Debug
...
window.loadCustomModel = async (path) => {
  if (!path || path === 'nessuno' || path === '') return;

  // Cleanup previous model
  if (window.customModel) {
    scene.remove(window.customModel);
    window.customModel.traverse((child) => {
      if (child.isMesh) {
  ...
  try {
    ...
    model.position.set(params.modelX, params.modelY, params.modelZ);
    ...
    if (params.debug) {
      modelTransformControls.attach(model);
    }
  } catch (error) {
    console.error("Errore nel caricamento del modello:", error);
    alert("Errore nel caricamento del modello. Controlla il link o il file locale.");
  }
};
...
// Columns — arranged in a semicircle around the pool and along the corridor
{
  ...
  for (let i = 0; i < 0; i++) {
    const angle = Math.PI * 0.-45 + (Math.PI * 0.7 / 4) * i
...
const line1 = createCenteredText('', textLineHeight)
const line2 = createCenteredText('', 0)
...
// Additional transform controls for the custom model
const modelTransformControls = new TransformControls(camera, debugOverlay)
modelTransformControls.addEventListener('dragging-changed', (event) => {
    controls.enabled = !event.value && params.debug
})
// Update params when gizmo moved
modelTransformControls.addEventListener('objectChange', () => {
    if (window.customModel) {
        params.modelX = window.customModel.position.x;
        params.modelY = window.customModel.position.y;
        params.modelZ = window.customModel.position.z;
        // Optionally update GUI via some helper if available, for now just update params
    }
})
const modelTransformHelper = modelTransformControls.getHelper()
...
function guiInput(parent, label, value, onChange) {
  const row = document.createElement('div')
  row.className = 'gui-row'
  const lbl = document.createElement('label')
  lbl.textContent = label
...
function guiButton(parent, label, onClick) {
  const row = document.createElement('div')
  row.className = 'gui-row'
...
// ── Custom Model folder
const customModelF = guiFolder('Custom Model')
guiSlider(customModelF, 'Position X', params.modelX, -50, 50, 0.1, (v) => { 
  params.modelX = v; 
  if (window.customModel) window.customModel.position.x = v;
})
guiSlider(customModelF, 'Position Y', params.modelY, -10, 20, 0.1, (v) => { 
  params.modelY = v; 
  if (window.customModel) window.customModel.position.y = v;
})
guiSlider(customModelF, 'Position Z', params.modelZ, -50, 20, 0.1, (v) => { 
  params.modelZ = v; 
  if (window.customModel) window.customModel.position.z = v;
})
guiSlider(customModelF, 'Scale', params.modelScale, 0.01, 10, 0.01, (v) => { 
  params.modelScale = v; 
  if (window.customModel) window.customModel.scale.setScalar(v);
})
guiCheckbox(customModelF, 'Edit Gizmo', false, (v) => {
    if (!window.customModel) return;
    if (v) {
        modelTransformControls.attach(window.customModel);
    } else {
        modelTransformControls.detach();
    }
})

// New On-Demand Loading Section
let localModelsF = guiFolder('Local Selection')
guiSelect(localModelsF, 'File locali', params.localModels, params.selectedLocalModel, (v) => {
  params.selectedLocalModel = v;
})
guiButton(localModelsF, 'Carica Modello Locale', () => {
    window.loadCustomModel(`./${params.selectedLocalModel}`);
})

let urlModelsF = guiFolder('Remote URLs')
guiInput(urlModelsF, 'Slot 1', params.urlSlot1, (v) => params.urlSlot1 = v)
guiButton(urlModelsF, 'Carica Link 1', () => window.loadCustomModel(params.urlSlot1))

guiInput(urlModelsF, 'Slot 2', params.urlSlot2, (v) => params.urlSlot2 = v)
guiButton(urlModelsF, 'Carica Link 2', () => window.loadCustomModel(params.urlSlot2))

guiInput(urlModelsF, 'Slot 3', params.urlSlot3, (v) => params.urlSlot3 = v)
guiButton(urlModelsF, 'Carica Link 3', () => window.loadCustomModel(params.urlSlot3))
...
let debugToggleInput
{
  ...
  debugToggleInput = guiCheckbox(debugF, 'Orbita3D', params.debug, setDebug)
}
...
// Listen for the elegant Orbita3D button click
document.getElementById('orbit-toggle-btn')?.addEventListener('click', () => {
  setDebug(!params.debug)
})
